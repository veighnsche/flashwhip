...
func (d *DB) SaveMessage(sessionID, role, content string) error {
	...
	if !exists {
		...
		_, err = d.sqlDB.Exec("INSERT INTO sessions (id, title, created_at, updated_at) VALUES (?, ?, ?, ?)", sessionID, title, now, now)
		if err != nil {
...
func (d *DB) GetSession(sessionID string) (*Session, []Message, error) {
	...
	var s Session
	err := d.sqlDB.QueryRow("SELECT id, title, created_at, updated_at FROM sessions WHERE id = ?", sessionID).Scan(&s.ID, &s.Title, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
...
func (d *DB) ListSessions() ([]Session, error) {
	...
	rows, err := d.sqlDB.Query("SELECT id, title, created_at, updated_at FROM sessions ORDER BY updated_at DESC")
	if err != nil {
